	...
	public class CDR_Mapper extends Mapper<LongWritable, Text, CompositeKey, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			 ...
			 try { 
				...
				String cel_id = CDR_data_[5].trim();
				String activity_type = CDR_data_[6].trim();
				String cel_tower_name = CDR_data_[7].trim();
				String lac = CDR_data_[8].trim();
				String cel_id1 = CDR_data_[9].trim();
				String latitude = CDR_data_[10].trim();
				String longitude = CDR_data_[11].trim();
				System.out.println(latitude);
				
				Long unix_time = DateParser.timeToUnixTime(s_timestamp);
				
				 CompositeKey stockKey = new  CompositeKey(imei_, unix_time);
				Text stockValue = new Text("," +imsi_ + "," + s_timestamp + "," + e_timestamp+ "," +lac+ "," +cel_id+ "," +activity_type+ "," +cel_tower_name+ "," +latitude+ "," +longitude);
